...
repository-scope skills. Claude Code does not read that directory on its own. A
recent irori supplies the chosen skill with the request; an older one does not,
so read `.agents/skills/<name>/SKILL.md` directly when you need one and it was
not given to you. See [ADR 001](docs/decisions/001-kb-structure.md) (D9).
